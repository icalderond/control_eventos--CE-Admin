...
try
{
	...
	{
        $sql = "SELECT 
        alumno.NoControl AS 'NoControl',
        alumno.Nombre AS 'Nombre',
        CONCAT (alumno.Apellido_Paterno,alumno.Apellido_Paterno) AS 'Apellidos',
         perfil.TallaCamisa AS 'Talla',
        perfil.Manga AS 'Manga',
        control_inscripcion.Total_Pagado AS 'TotalPago'
        FROM alumno
        LEFT JOIN perfil ON perfil.alumno_NoControl=alumno.NoControl
        LEFT JOIN control_inscripcion ON control_inscripcion.Id_Control=alumno.NoControl";
